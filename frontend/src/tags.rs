...
#[component]
fn TagRow(cx: Scope, tag: TagEntry, archived: bool) -> Element {
    let url = use_shared_state::<TagsUrl>(cx).unwrap();
    let tags = use_shared_state::<TagEntries>(cx).unwrap();
                    ...
                    div {
                        ...
                        if *archived {rsx! {
                            svg {
                                ...
                                ",
                                onclick: |_| set_tag_archived(tags.clone(), url.clone(), tag.id, false),
                                path {
                                    "stroke-linecap": "round",
                                    "stroke-linejoin": "round",
...
async fn send_get_archived_tags_request(url: &Url) -> Result<Vec<TagEntry>, anyhow::Error> {
